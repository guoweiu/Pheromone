...
export STORE_SERVICE_CONFIG_PATH=${service_config_file}; \
/tmp/SPheromone126/coordinator/coordinator 1>/tmp/sp02/coordinator.log 2>&1 & \
/tmp/SPheromone126/scheduler/scheduler 1>/tmp/sp02/scheduler.log 2>&1 &"
...
ssh -p 22 lgw@192.168.1.126 "for ((i = 0; i < ${thread_num}; i++)); do export CONFIG_FILE=${config_file_126}; \
   ...
   export LUMINE_GATEWAY_ADDR=192.168.1.129; \
   export LUMINE_GATEWAY_PORT=8084; \
   /tmp/SPheromone126/executor/executor \$i 1>/tmp/sp02/executor_\${i}.log 2>&1 & done"
